...
const StudentCard: React.FC<StudentCardProps> = ({ student, onChangeStatus, onDownloadId, onDelete, basePath = "/admin/students" }) => {
  ...
  return (
    <div className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900/60">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-sm font-medium text-blue-600 dark:text-blue-300">{student.standard?.name ?? "No standard assigned"}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-900/20 dark:text-blue-300">{student.standard?.name ?? "No standard"}</span>
      ...
      <div className="mt-5 flex items-center gap-2">
        <Link href={`${basePath}/${student.id}`} className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
          <Eye size={16} className="inline-block" /> View
        </Link>
        <Link href={`${basePath}/${student.id}/edit`} className="rounded-xl bg-blue-600 px-3 py-2 text-sm font-medium text-white transition hover:bg-blue-700">
