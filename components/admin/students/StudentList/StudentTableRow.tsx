...
const StudentTableRow: React.FC<StudentTableRowProps> = ({ student, selected, onToggleSelect }) => {
  return (
    <tr className="border-b border-slate-100 transition hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-900/70">
      ...
      <td className="px-4 py-4">
        <div className="flex items-center gap-2">
          <Link href={`/admin/students/${student.id}`} className="rounded-lg p-2 text-slate-600 transition hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"><Eye size={16} /></Link>
          <Link href={`/admin/students/${student.id}/edit`} className="rounded-lg p-2 text-slate-600 transition hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"><Edit3 size={16} /></Link>
