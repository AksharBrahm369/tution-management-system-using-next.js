...
export default function SecuritySettings({ settings, onSaved }: Props) {
  ...
  return (
    <section className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
      ...
      <div className="flex justify-end"><button type="button" onClick={save} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-60"><ShieldCheck size={16} /> {saving ? "Saving..." : "Save Security"}</button></div>
