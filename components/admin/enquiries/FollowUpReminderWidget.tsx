...
export default function FollowUpReminderWidget() {
  ...
  return (
    <div className="flex h-full min-h-[260px] flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="mt-3 flex flex-1 flex-col space-y-2">
        {isLoading ? (
          ...
          data?.followUps.map((item) => (
            <div key={item.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/40">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="truncate font-medium text-slate-900 dark:text-white">{item.studentName}</div>
                  <div className="truncate text-xs text-slate-500">{item.enquiryNumber} / {item.parentName}</div>
                </div>
                <span className="shrink-0 rounded-md bg-amber-50 px-2 py-0.5 text-[11px] font-medium text-amber-700 ring-1 ring-inset ring-amber-100 dark:bg-amber-950/40 dark:text-amber-300 dark:ring-amber-900/50">
          ...
          <div className="flex min-h-[62px] items-center rounded-lg border border-dashed border-slate-300 px-4 py-3 text-sm text-slate-500 dark:border-slate-700">No follow-ups due right now.</div>
        )}
      </div>
    </div>
  );
