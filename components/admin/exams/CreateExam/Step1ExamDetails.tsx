  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900/60">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm"><span className="text-slate-500">Exam Category*</span><select className="mt-1 w-full rounded-lg border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" value={form.type} onChange={(e) => set("type", e.target.value)}>{examTypes.map((type) => <option key={type} value={type}>{type.replaceAll("_", " ")}</option>)}</select></label>
