...
export default function Step1ExamDetails({ form, batches, subjects, onChange, onNext }: { form: CreateExamForm; batches: BatchOption[]; subjects: SubjectOption[]; onChange: (form: CreateExamForm) => void; onNext: () => void }) {
  ...
  const examTypes = ["UNIT_TEST", "MID_TERM", "FINAL", "MOCK_TEST", "CLASS_TEST", "ASSIGNMENT", "PRACTICAL"] as const;
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900/60">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <label className="text-sm"><span className="text-slate-500">Exam Title*</span><input className="mt-1 w-full rounded-lg border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" value={form.title} onChange={(e) => set("title", e.target.value)} required aria-required="true" />{errors.title && <div className="text-sm text-red-600 mt-1">{errors.title}</div>}</label>
        ...
        <label className="text-sm"><span className="text-slate-500">Exam Category*</span><select className="mt-1 w-full rounded-lg border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" value={form.type} onChange={(e) => set("type", e.target.value)} required aria-required="true">{examTypes.map((type) => <option key={type} value={type}>{type.replaceAll("_", " ")}</option>)}</select></label>
        <label className="text-sm"><span className="text-slate-500">Batch*</span><select className="mt-1 w-full rounded-lg border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" value={form.batchId} onChange={(e) => { const batch = batches.find((item) => item.id === e.target.value); onChange({ ...form, batchId: e.target.value, subjectId: batch?.subjectId ?? form.subjectId }); }} required aria-required="true"><option value="">Select batch</option>{batches.map((batch) => <option key={batch.id} value={batch.id}>{batch.name}</option>)}</select>{errors.batchId && <div className="text-sm text-red-600 mt-1">{errors.batchId}</div>}</label>
        <label className="text-sm"><span className="text-slate-500">Subject*</span><select className="mt-1 w-full rounded-lg border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" value={form.subjectId} onChange={(e) => set("subjectId", e.target.value)} required aria-required="true"><option value="">Select subject</option>{subjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select>{errors.subjectId && <div className="text-sm text-red-600 mt-1">{errors.subjectId}</div>}</label>
        <label className="text-sm"><span className="text-slate-500">Academic Year*</span><input className="mt-1 w-full rounded-lg border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" value={form.academicYear} onChange={(e) => set("academicYear", e.target.value)} required aria-required="true" /></label>
        <label className="text-sm"><span className="text-slate-500">Exam Date*</span><input className="mt-1 w-full rounded-lg border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" type="date" value={form.examDate} onChange={(e) => set("examDate", e.target.value)} required aria-required="true" />{errors.examDate && <div className="text-sm text-red-600 mt-1">{errors.examDate}</div>}</label>
