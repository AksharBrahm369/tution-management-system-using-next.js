...
import { Users, LayoutDashboard, ShieldAlert, ArrowLeft } from "lucide-react";
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (loggedUser && loggedUser.role === "PARENT") {
          setAuthError(null);
        } else {
          setAuthError("FORBIDDEN");
        }
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="mt-2.5 text-sm text-slate-500 dark:text-slate-400 leading-relaxed px-2">
              {authError === "UNAUTHORIZED" ? (
                "You are not currently logged in. Please log in with your parent credentials to view this dashboard."
              ) : (
                "You are currently logged in with a non-parent account. To access this portal, please log in with a Parent account."
              )}
            </p>
            ...
            <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-medium text-slate-500 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-400">
              Need parent access? Contact your administrator to get credentials.
            </div>
            ...
            <div className="mt-6 flex flex-col gap-2.5 w-full">
              ...
              >
                {loggingOut ? "Signing out..." : "Log In as Parent"}
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 pb-16 md:pb-0">
      ...
      <header className="hidden md:flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4 dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center gap-8">
          ...
          <Link href="/parent/dashboard" className="text-xl font-semibold text-blue-700 dark:text-blue-300">
            TuitionPro Parent
          </Link>
          <nav className="flex gap-6">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`text-sm font-medium ${pathname === item.href || pathname.startsWith(item.href) ? "text-blue-700 dark:text-blue-300" : "text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"}`}
              >
                {item.name}
              </Link>
            ))}
      ...
      <header className="md:hidden flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900 sticky top-0 z-10">
        ...
        <div className="text-base font-semibold text-blue-700 dark:text-blue-300">TuitionPro Parent</div>
        <div className="w-9" />
      </header>
